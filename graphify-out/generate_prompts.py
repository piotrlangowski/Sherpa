...
images = [f for f in uncached if Path(f).suffix.lower() in ['.svg', '.png', '.jpg', '.jpeg', '.gif']]
...
sorted_non_images = []
for p in sorted(by_dir.keys()):
    sorted_non_images.extend(by_dir[p])
...
# Split non-images into chunks of 22
for i in range(0, len(sorted_non_images), 22):
    chunks.append(sorted_non_images[i:i+22])

# Write prompts and paths info
project_root = Path('graphify-out/.graphify_root').read_text(encoding='utf-8').strip()
prompts_info = []

for idx, chunk in enumerate(chunks):
    chunk_num = f'{idx+1:02d}'
    chunk_path = f'{project_root}/graphify-out/.graphify_chunk_{chunk_num}.json'
    file_list_str = '\n'.join(chunk)
...
Files (chunk {idx+1} of {len(chunks)}):
{file_list_str}
...
Path('graphify-out/.graphify_prompts.json').write_text(json.dumps(prompts_info, indent=2), encoding='utf-8')
print(f'Wrote {len(prompts_info)} prompts to graphify-out/.graphify_prompts.json')

...
async function test() {
  ...
  const baseScenario = db.prepare("SELECT id, name FROM scenarios LIMIT 1").get() as any;
  if (!baseScenario) {
  ...
  const loaded = db.prepare("SELECT * FROM scenarios WHERE id = ?").get(scenarioId) as any;
  if (!loaded) {

...
Exposes 15 consolidated entity-action tools covering full database CRUD operations for all entities (settings, providers, verticals, cohorts, fixed costs, feature packs, pricing plans, scenarios, scenario cohort overrides, per-scenario entity financial overrides, monetization models, and credit-pool tiers) with action parameters and Human-in-the-Loop confirmations, plus dashboard open/close controls.
...
- **One revenue carrier / Composite Carrier (ADR 0001–0004, amended by ADR 0014).** Each scenario sets a `modeling_type` (`incremental` | `gtm` | `appraisal` | `composite`) and a `revenue_carrier` (`cohort` | `plan` | `pack` | `feature` | `pool` | `composite`). By default, exactly one carrier books revenue, preventing double-counting. Under ADR 0014, a `composite` carrier is introduced to model multiple overlapping mechanisms (Cohort ARPU, Plan seats, Services, and Pools) simultaneously. Double-counting in `composite` is prevented via automatic folding based on relationship classifications (`PerspectiveRelation`): Cohort-Plan upsell folding (`revenue_bridge === 'upsell_on_cohort'`), Copilot folding into ARPU uplift (`arpu_uplift_includes_monetization === true`), and Pool-Service exclusive billing containment.
- **Per-archetype streams + credit pool (ADR 0009–0010, amended by ADR 0012 & 0014).** Copilot (seat) and agent (interaction) are disjoint, summable streams. An unmonetized agent books labor savings as cash, while a monetized agent books its service fee as revenue (with labor savings as memo only). Credit Pool billing is governed by `fee_basis` and `pool_size_basis`. In composite mode, the credit pool selector is optional. If the target seats differ from the cohort population base by a factor of >1.2, the relationship is flagged as `incommensurable` ("mierzy inny rynek").

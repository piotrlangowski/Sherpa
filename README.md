...
### Value-Based Pricing (EVC) & Per-Cohort Differentiation

The **Pricing Corridor** chart (`COGS Floor → Price → EVC Ceiling`) shows, per cohort, how a scenario's realized price sits between its cost floor and the value it creates. Historically the EVC ceiling/target/floor were computed once from three scenario-level scalars (`evc_nba_annual_value`, `evc_extra_positive_value`, `evc_negative_value`), so every cohort saw an identical ceiling regardless of how much its `base_arpu` or `usage_intensity` actually differed — a real modeling gap, since willingness-to-pay plausibly varies a lot by segment. Two archetype-specific fixes close it, both by extending override mechanisms that already exist rather than adding new base-entity columns (see [ADR 0009](Architecture/0009-strumienie-przychodu-per-archetyp.md) and [ADR 0011](Architecture/0011-evc-per-segment.md)):

- **Copilot (willingness-to-pay).** A scenario picks an `evc_reference_cohort_id`; every other cohort's ceiling is then scaled from it via three decomposed multipliers — `evc_extra_value_multiplier` (productivity value), `evc_negative_value_multiplier` (switching cost/friction), `evc_nba_multiplier` (next-best-alternative cost) — resolved through the same global → vertical → cohort scope-override cascade used for `gross_margin`/`churn_reduction`/etc. A data-derived suggested default (`sqrt(base_arpu ratio × usage_intensity ratio)`) avoids a blind first guess, returning the explicit underlying ratios for transparency. If `revenue_carrier` resolves to `'cohort'`, the chart displays the actual per-cohort price rung (`pricePerCustomer`) instead of the blended average. `null`/unset multipliers preserve the original flat ceiling — fully backward compatible.
- **Agent (displaced-labor cost).** A willingness-to-pay ceiling isn't meaningful for a deflection product — its value is the cost of the human/process alternative it avoids. `scenario_entity_overrides` uses an optional `cohort_id` scope so labor-cost/complexity fields (`fully_loaded_cost_per_fte_month`, `containment_rate`, `average_handle_time_seconds`, `interactions_per_customer_month`, `churn_rate_uplift`) can vary per cohort instead of once per scenario, resolved inside the engine's `per_customer` interaction-driver path. The result surfaces as a parallel **Cost-to-Serve** chart (Avoided Value ↔ Cost to Serve), shown alongside the Pricing Corridor whenever the scenario has an agent-archetype interaction stream. Avoided value is scaled by the ratio of actual FTEs saved over theoretical capacity savings (`realizationRatio`), visualised as an amber circular marker when a baseline staffing cap is configured. The chart tooltips display the detailed breakdown of cost components (tokens, fixed allocation, failed deflection).
